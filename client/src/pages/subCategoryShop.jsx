...
const SubCategoryShop = () => {
  const { id: subCategoryId } = useParams(); // Extract subcategory ID from route
  const [products, setProducts] = useState([]); // Store fetched products
  const [subCategoryName, setSubCategoryName] = useState(""); // Store subcategory name
  ...
  useEffect(() => {
    // Fetch subcategory products dynamically
    const fetchSubCategoryProducts = async () => {
      setLoading(true);
      try {
        const response = await axios.get(
          `/category/getSubCategory/${subCategoryId}`,
          {
            params: { page: currentPage, limit: pageSize },
          }
        );
        const { subCategory } = response.data;
        setProducts(subCategory.product || []);
        setSubCategoryName(subCategory.name || "SubCategory");
        setTotalProducts(subCategory.product?.length || 0); // Adjust total products if needed
        setError(null);
      } catch (err) {
        ...
        <div className="w-full md:p-4 p-0">
          ...
          >
            {sortedProducts.length > 0 ? (
              ...
              <div className="text-white">No products found in this subcategory.</div>
            )}
          </div>
