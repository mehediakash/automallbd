...
const BannerAdd = () => {
  ...
  useEffect(() => {
    const fatchBanner = async () => {
      ...
      let banneradd = res.data.banners.filter(
        (secondBannerLeft) => secondBannerLeft.position == "SecondBannerLeft",
      );
      let rightBannerAdd = res.data.banners.filter(
        (secondBannerRight) =>
